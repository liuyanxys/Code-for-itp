...
BEAM_res <- BEAM(my_cds_subset[expressed_genes, ], branch_point =1, cores = 5)
BEAM_res <- BEAM_res[order(BEAM_res$qval),]
BEAM_res <- BEAM_res[,c("gene_short_name", "pval", "qval")]
BEAM_res <-subset(BEAM_res,qval < 1e-4)
save(BEAM_res,file='BEAM_res_my_cds_subset_yoursample_MkEry_500.Rdata')
#load(file='BEAM_res_my_cds_subset_yoursample_MkEry_500.Rdata')
...
pdf("BEAM700-branch1-4clusters.pdf", width = 10, height = 12)
plot<-plot_genes_branched_heatmap(my_cds_subset[BEAMgene,],
                                          ...
                                          show_rownames = F)
dev.off()
save(plot,file="BEAM-branch1-4clusters.Rdata")
annotation_row<-plot$annotation_row
write.table(annotation_row, "BEAM-branch1-4clusters-annotation_row.csv", row.names=T, sep='\t', append=F, quote=F)

###GO enrichment for BEAM
#load("BEAM-branch1-4clusters.Rdata")
annotation_row<-plot$annotation_row
...
write.table(go, "BEAM-branch1-4cluster-annotation_row-GO.csv", row.names=T, sep='\t', append=F, quote=F)
